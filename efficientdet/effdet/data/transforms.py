...
class CustomValTransforms:
        def __call__(self, pil_img, annotations: dict):
            np_img = np.array(pil_img, dtype=np.uint8)
            np_img = np.transpose(np_img, (1, 2, 0))

            transform = A.Compose([
                tf.VerticalFlip(p=0.5),
            ...
            new_img = np.transpose(transformed_image, (2, 0, 1))
            return new_img, annotations


class CustomTrainTransforms:
    def __call__(self, pil_img, annotations: dict):
        np_img = np.array(pil_img, dtype=np.uint8)
        np_img = np.transpose(np_img, (1, 2, 0))
        ...
        new_img = np.transpose(transformed, (2, 0, 1))
        return new_img, annotations
